...
def check_data_quality_drift(ref_df: pd.DataFrame, curr_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    return {
        "quality_drift_detected": len(issues) > 0,
        "issues": issues
    }
...
def detect_concept_drift_comprehensive(
    ref_X: pd.DataFrame, ref_y: pd.Series, 
    curr_X: pd.DataFrame, curr_y: pd.Series, 
    model, 
...
def detect_data_drift(
    ...
    if model is not None and target_col is not None and target_col in df_sorted.columns and feature_cols is not None:
        concept_report = detect_concept_drift_comprehensive(
            ref_df[feature_cols], ref_df[target_col],
            curr_df[feature_cols], curr_df[target_col],
            model, mae_threshold
        )
